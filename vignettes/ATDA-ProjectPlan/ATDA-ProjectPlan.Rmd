...
\fcolorbox{black}{white}{
	\parbox[t]{1.0\linewidth}{
		...
		\vspace*{0.7cm}
    ATDA-ASR`r r6ob_abbrtable$add_abbrev(psAbbrev = "ATDA-ASR", psMeaning = "Projekt von ASR zur Automatisierung des Tierdatenaustauschs", pbOut = FALSE)` \\
...
`r r6ob_abbrtable$add_abbrev(psAbbrev = "FB EDV", psMeaning = "Fachbereich elektronische Datenverarbeitung", pbOut = FALSE)`
...
objSecEnum$displayNumSection()
```
`NDX` steht für Nordic Data Exchange`r r6ob_abbrtable$add_abbrev(psAbbrev = "NDX", psMeaning = "Nordic Data Exchange", pbOut = FALSE)`. Dies soll ein Projekt sein, welches den Datenaustausch in den skandinavischen Ländern automatisiert. Von Seiten der Hersteller sind Lely und DeLaval beteiligt. 
...
```{r Diagramm_Milchdatenfluss_ATDA}
...
```{r echo=FALSE,results='asis'}
objSecEnum$setUnNumSection(psUnNumSection = 
"## Austausch mit CRV Holland")
objSecEnum$parseUnNumSection()
objSecEnum$displayNumSection()
```
Anlässlich des Treffens mit den Melkanlagenanbietern am 18.3.2016 erfuhren wir, dass CRV Holland mit den Firmen DeLaval, Lelly und Fullwood bereits ein Service eines automatischen Tierdatenaustauschs betreibt. Zur Vermeidung von Doppelspurigkeiten und zur Minimierung des Aufwandes in diesem Projekt für die Melkanlagenanbieter und für uns, haben wir Kontakt zu `r r6ob_abbrtable$add_abbrev(psAbbrev = "BvL", psMeaning = "Bert van t Land")` von CRV Holland aufgenommen (Telefonkonferenz am 19.05.2016, siehe dazu Abschnitt 1.3.2). Zusammenfassend werden im Datenaustauschprojekt in Holland von rund 3000 Betrieben mit Robotermelkanlagen täglich mehr als 1 Mio Milchmengenrecords von den Betrieben auf die zentrale Datenbank übertragen. Unter Berücksichtigung eines Datenoverheads, entspricht dies einer Datenmenge von rund 100MB pro Tag. Die Milchmengen jedes Gemelks werden in der zentralen Datenbank abgelegt. Nach der Ablage eines Milchmengenrecords wird ein Tagesdurchschnitt der Milchmengen über die vorangegangenen 24 Stunden berechnet. Die Tagesdurchschnitte werden auf eine Laktationsmilchmenge hochgerechnet.

Das Betreiben der zentralen Datenbank sowie die Programmierung von ISM-spezifischen PlugIns wurde von CRV zu VSM - einer externen Informatikfirma - ausgelagert. Die PlugIns werden auf den ISM jedes Betriebs installiert. Dort steuern sie die Übertragung der Daten von den Betrieben zur zentralen Datenbank. 
...
objSecEnum$displayNumSection()
```
Bei der Beschreibung des Datenexports in diesem Dokument (Abschnitt 3.1.1) liegt die Verantwortung des Datenexports bei den Melkanlagenanbietern. Im Gegensatz dazu, wird im Datenübertragungs-Projekt von CRV-Holland der Datenexport und die Übertragung von einem durch VSM geschriebenes On-Farm-PlugIn übernommen. Das PlugIn, welches die Datenübertragung steuert, muss spezifisch für jeden Anbieter der ISMs angepasst sein. Das heisst es braucht für jeden ISM-Anbieter ein anderes PlugIn. Diese Anforderung der spezifischen PlugIns würde sich auch für unser Projekt `ATDA-ASR` ergeben, da die in der Schweiz eingesetzten ISMs unterschiedliche Datenmanagementtechnologien verwenden. Die Tatsache, dass unterschiedliche ISM-Anbieter unterschiedliche Technologien einsetzen, wird durch die Ergebnisse unserer Befragung der Melkanlagenanbieter bestätigt. 

BvL hat sich bereit erklärt, die Ansprechsperson für Fragen betreffend des holländischen Datenaustauschprojekts zu sein. Er klärt bei CRV und VSM ab, ob und in welchem Umfang eine Zusammenarbeit zwischen diesem Projekt `ATDA-ASR` und CRV-Holland möglich ist. Für uns ist es wichtig die Frage zu klären, ob die OnFarm-PlugIns der verschiedenen Anbieter, welche in Holland eingesetzt werden auch auf den Betrieben in der Schweiz eingesetzt werden können. BvL ist der Meinung, dass diese nicht ohne Anpassungen in der Schweiz verwendet werden können. 

Falls die PlugIns aus dem Projekt von CRV in Holland für uns nicht verfügbar sind, müssen wir die Funktionalität des Datenexports aus dem ISM selber umgesetzen. Für eine eigene Umsetzung zuerst die von den verschiedenen Melkanlagen zur Verfügung gestellten Informationen analysiert werden. Die Beschreibung der Daten der verschiedenen Anbieter sind in separaten Dokumenten abgelegt.
...
\blandscape
```{r ProjektAblaufplanQ4}
rmddochelper::insertOdgAsPdf(psOdgFileStem = "ProjektAblaufplanQ4", 
                             psOdgDir = "odg/ATDA-ProjectPlan",
                             pnPaperWidthScale = 1.5)
```
\elandscape
